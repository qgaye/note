...
分区表对于Server层来说就是一张表，而在引擎层中则根据用户设定会分成多张表(即只有一个.frm文件和多个.ibd文件)

当给分区表加表锁和行锁时会出现一些问题，因为对于Server层来说只有一张表，但在加锁是由引擎层完成的，在实际加锁上会对对应的分区表加锁，因此会存在引擎层加锁的范围和Server层需求的范围不同的情况

### 分区策略

- 在MyISAM中使用的是通用分区策略，即由Server层控制，而在MySQL启动时就需要将所有分区遍历一遍，但如果所有分区数量大于了`open_files_limit`设定的值，那么就会由打开表文件的数量超过上限报错(在MySQL8.0中已经不支持创建MyISAM的分区表了)
- 在InnoDB中使用的是本地分区策略，即由InnoDB自行打开管理表分区行为，就不会因为超过`open_files_limit`设定的值而报错了
...
优点：
- 使用分区表对业务透明，相对于业务层分表来说，使用分区表的业务代码更简洁
- 分区表可以很方便的清理历史数据，可以直接通过`alter table t1 drop partition...`来删除历史数据
