...
在同步代码块起始位置插入moniterenter指令，结束位置插入monitorexit指令，本质作用就是来找到这个对象的monitor，然后把加锁/释放锁交给Monitor去做

![Monitor](../pics/monitor.png)

膨胀后的重量级锁，用ObjectMonitor类实现，即Monitor，Monitor本身有Owner/EntryList/WaitList三个部分，owner表示持有的线程，entry表示等待的线程，wait表示被wait()的线程

Mark Word中有指向Monitor的指针()

synchronzied底层依赖于一个monitor对象来完成加锁/wait/notify作用
