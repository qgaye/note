...
缓存行(Cache Line)：是CPU缓存中的最小单位，CPU缓存由若干缓存行组成，一个缓存行大小通常为64字节(这取决于CPU)

伪共享(False Sharing)：不同的CPU核并发读写同一变量，导致别的CPU核下的缓存行失效，尽管不同CPU核读写的变量之间没有任何关系
...
比如T结构体中的变量X和Y可能会被缓存在同一缓存行中，一旦一个CPU核修改了变量X，就会触发缓存一致性协议，导致其他CPU核中的缓存行失效，从而不得不从上一级缓存中重新加载该缓存行，这个开销是极其昂贵的，并且其他CPU核可能根本不需要读取变量X，只需要读取变量Y，也就是说这可能是一次无意义的缓存重新加载

规避伪共享的方式：在T结构体中为变量X和Y之间加上padding，即将变量X和Y分别加载到不同的缓存行中即可

```go
type struct T {
  X uint64  // 8字节
  padding [7]uint64  // 56字节的padding
  Y uint64  // 8字节
}
```

Java8中可以通过`@Contended`注解来避免伪共享，但因为这会增加padding从而增加目标实例大小，默认情况下除了JDK内部的类，JVM会忽略该注解

缓存一致性和可见性区别：

缓存一致性由底层硬件实现，缓存一致性中的一致性指的是主存和CPU缓存中的一致性。当主存中某个变量值变化后，缓存中的值也会被刷新，刷新可能是指将该指标记为失效，下次主动从主存中获取，也可能是直接用新值覆盖。通过总线窥探(Bus Snooping)实现主存的数据变化被CPU核监听到，只要监听总线操作就可知道别的CPU核对主存的操作情况

而可见性指的是一个线程对某个变量值修改时，其他线程都能够看到，总结来说：缓存一致性指的是主存和缓存的一致，而可见性指的是缓存和缓存间的一致，因为当一个线程修改变量值后写入一级缓存后，并不一定直写回主存，可能采取回写机制，即先标注为在脏区域，然后后台同步到主存，采用回写主要是因为写入主存太慢了

如如何实现可见性：

只要立刻写入主存，然后通过底层提供的缓存一致性就可以实现可见性，通过防止指令重排列，这样写入主存的操作就会立即被执行，一般通过加内存屏障，比如synchronized，volatile都是依赖内存屏障的
