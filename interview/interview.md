...
阻塞IO：同步阻塞，在准备数据和读取数据都阻塞
非阻塞IO：同步非阻塞，不断轮询
IO多路复用：同步阻塞，可以同时监听多个文件信息
异步IO：异步非阻塞
...
在接收到3个以上的重复ACK后将慢启动阈值设为原来一半，接着执行拥塞避免算法，每次+3增大
...
在客户端进行最后一次挥手后会启动一个等待定时器，在等待2MSL(MSL为最长报文段寿命，通常为2分钟)，其作用是因为最后一次挥手后服务端是没有确认回复的，因此客户端需要确认服务器接收到了客户端的挥手，客户端在发起断开连接后在2MSL中如果没有收到确认，会重传，此时客户端处在等待2MSL中，因此就能回答。此外等待2MSL还能保证整个连接中的报文段都过期了
...
通用首部字段(Date：创建报文时间，Connection：连接的管理，Transfer-Encoding：报文主体的传输编码方式)
...
HTTP 1.1: 默认使用connection: keep-alive，使得TCP连接建立后可以重复发生请求

HTTP 2: 
- 使用二进制传输数据，而不再是文本形式
- 多路复用：HTTP2最小单位变成了帧，通过二进制分帧，因此不再以来TCP来实现多流并行，同一个域名只需要一个TCP连接(以前要6个)，且帧还可以乱序发送
- 服务器可以主动给客户端推送内容
- 头部压缩
...
红黑树：AVL树这颗树更平衡，查找快，插入慢，但AVL树的旋转是O(lgn)，而红黑树只需要两次，因此综合选择红黑树
...
1. 利用覆盖索引，减少回表
2. 如果主键是递增的(auto_increment)，那么可以先查出主键，然后用in，join，between，> 方式查出具体数据
