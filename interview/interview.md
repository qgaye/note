...
- synchronzied是关键字，Lock是接口
- JVM会在代码段结束或发送异常后自动释放锁，而Lock接口需要自己手动释放(lock有编程规范)
- Lock可以让等待锁的线程响应中断，tryLock方法，而synchronzied不可以
- ReententLock底层原理是AQS，synchronzied底层原理是Monitor
...
### ClassLoader和Class.forClass

Class.forClass()会完成加载-链接-初始化三步，即static变量会被初始化并会执行static代码块(JDBC中需要forName是因为需要执行static代码块向DriverManager注册自己)
...
可以在设置缓存失效时间上在基础缓存时间上加上随机树，从而保证缓存不是同时失效
