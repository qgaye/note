...
- 默认使用Connection: keep-alive，使得TCP连接建立后可以被复用
...
- 将传输的信息分割为帧，并采用二进制进行编码
- 以上两种特质使得在HTTP2.0中可以将多个请求分到不同的流中，然后拆分成帧以二进制形式乱序发送

HTTP2.0解决了1.1中队首阻塞问题(1.1中需要串行发送，而2.0中可以分到不同的流中并行发送)，也无需使用HTTP1.1中pipline机制创建多条TCP(Chrome中默认同时创建6条)来实现并行请求，减少了TCP连接数从而降低了对服务器性能的影响
