...
HTTP2.0解决了1.1中队首阻塞问题(1.1中需要串行发送，而2.0中可以分到不同的流中并行发送)，也无需使用HTTP1.1中pipline机制创建多条TCP(Chrome中默认同时创建6条，6条是因为浏览器对同一域名下的请求连接有条数限制)来实现并行请求，减少了TCP连接数从而降低了对服务器性能的影响

队首阻塞问题：HTTP层面的队首阻塞问题是前一个请求未处理完后面的请求就会被阻塞，TCP层面的队首阻塞问题则是前一个报文没有收到是不会将后面的报文上交给HTTP的
