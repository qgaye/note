...
`ack=all`的实现方式：Leader副本的HW要等于ISR集合中所有Follower副本的LEO后才返回
...
- 挂载（mount）文件系统时禁掉access time的更新，避免inode访问时间的写入，减少文件系统的写操作
- 文件系统选择ext4或XFS
- swappiness=0表示操作系统将最大程度使用内存，除非内存不够用再用交换分区，这可以避免大量页的换进换出
- 页缓存越大越好，至少容纳一个日志段的大小，也就是broker端`log.segment.bytes`，这样保证消费程序在消费时能直接命中页缓存，避免磁盘IO
- 调大`vm.max_map_count`，因为一个LogSegment下就有4个物理文件，所以当topic或partition很多时就易出现Map failed内存映射失败

### JVM调优

- JVM堆大小设置为6-8G，或者Full GC后堆上存活的对象总和的2倍
- 使用G1收集器，相较于CMS收集器优化难度小

### Broker端调优

- 客户端和Broker端版本一致，否则消息需要JVM堆中进行中转，无法利用zero copy

### 应用层调优

- 不要频繁创建producer和consumer
- 用完及时关闭，对象底层很多物力资源不及时关闭会造成资源泄漏
- 合理利用多线程来改善性能，producer是线程安全的，consumer虽然不是线程安全的，但可以通过内部多线程处理解决

### 吞吐量调优

吞吐量（TPS） = 1 / 时延

通过较小的时延增加的代价，换取TPS的显著提升

- Broker端增大`num.replica.fetchers`值，即Follower副本用更多的线程来拉取消息
- Producer端调大`batch.size`和`linger.ms`，它们表示消息批次大小和批次缓存时间，调大producer可以一次性发送更多消息
- 如果目标是TPS，那么最好不要设置ack=all以及开启重试
- 多个线程共用一个producer实例，那么需要显式调大`buffer.memory`值，确保缓冲区空间足够
- Consumer端调大`fetch.min.bytes`，使broker积攒了更多的数据后再返回给consumer端

## 时延调优

- Producer端调小`batch.size`和`linger.ms`，将消息尽快发出去
- Consumer端调小`fetch.min.bytes`，broker端有消息就立刻返回给客户端进行消费

## 索引

- 位移索引：`<位移值，文件磁盘物理位置>`
- 时间戳索引：`<时间戳，位移值>`
- 事务索引：为已中止事务保存重要的元数据信息

位移索引中一个`<位移值，文件磁盘物理位置>`大小为8字节：位移值4字节，物理位置也是4字节，这里的位移值不是Long而是Int，是因为这里保存的是相对于该日志段的baseOffset的偏移值，从而有效节省磁盘空间，此外因为Broker端`log.segment.bytes`是整型，也就说明每个日志段大小不会超过2^32，即4G，所以和baseOffset偏移值也就不会超过4字节

时间索引中一个`<时间戳，位移值>`占12字节，其中时间戳8字节，位移值4字节

Kafka索引底层都是通过内存映射文件mmap实现的，即Java中的MappedByteBuffer，对于索引这样的小文件，由于文件内存直接被映射到了一段虚拟内存上，IO性能更高，此外对于Linux操作系统，这段映射的内存区域实际上就是内核的页缓存，这也就意味着里面的数据不需要重复拷贝到用户态空间

Kafka中索引查找使用的是二分查找，但其将数据分为热和冷两块，因为普通二分查找需要从头和尾两处开始，而头部数据很可能已经被换出页缓存了，也就会触发缺页中断，并且同时大部分读取的数据都是最近写入的数据，所以固定最近的8192字节的数据为热数据，当查询时首先判断该查询的索引项是否在热区中，如果在热区中就二分查找（热区的最前端，索引文件的末尾），否则就二分查找（0，热区的最前端）

为什么热区固定8kb：因为大部分CPU架构中页缓存都是4kb，8kb可以保证热区的页缓存数在三个以下，从而保证在二分查找时一次查询就可以查到，但固定8kb也存在问题，比如topic的读写QPS很低，及时热区的数据也可能被换出页缓存，所以最好自行实现缓存结构，类似MySQL
